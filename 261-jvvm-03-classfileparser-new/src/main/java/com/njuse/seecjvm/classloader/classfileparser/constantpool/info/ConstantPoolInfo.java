    ...
    public static Pair<ConstantPoolInfo, Integer> getConstantPoolInfo(ConstantPool constantPool, byte[] classfile, int ofs) {
        ...
        switch (tag) {
            ...
            case UTF8: {
                 ...
                 * Add some codes here.
                 */
                int position = in.getByteBuffer().position();
                Pair<UTF8Info, Integer> UTF8InfoPair = UTF8Info.getInstance(constantPool,classfile,position);
                ret = UTF8InfoPair.getKey();
                bytesRead += UTF8InfoPair.getValue();
                break;
            }
